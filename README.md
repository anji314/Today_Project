...
> (+) 라이브러리를 사용해 이슈를 해결했다. 해당 라이브러리는 로그인 기능만 지원을 해주기 때문에 우선 로그인 기능만 먼저 완성한 상태이다. 해결 방법은 아래 주소로 포스팅 해놨다.  


... 자세한 기록은 블로그에 => [해결과정 자세한 내용](https://footprint-of-nawin.tistory.com/37)              

... 로그인 cors 에러 해결 => [해결 방법](https://footprint-of-nawin.tistory.com/46)
